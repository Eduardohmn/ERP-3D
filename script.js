        ...
        if (inputDados) {
            ...
            if (tokenMatch) {
                ...
                if (GIST_ID.length > 0) {
                    ...
                    alert("⚠️ ID do Gist não encontrado. Iniciando offline."); return;
                }
            } else {
                alert("⚠️ Token 'ghp_' inválido. Iniciando offline."); return;
            }
        } else {
            return;
        }
...
// ==========================================
// ABA 1: SIMULAÇÃO DE CUSTOS (COM RENDIMENTO)
// ==========================================
...
function renderizarCatalogo() {
    ...
    DB.receitas.forEach(r => {
        ...
        <div class="item-card" style="border-left: 4px solid var(--primary);">
            ...
            <div class="item-details">
                <span style="color: var(--primary); font-weight: bold;">Custo Unitário Base: ${fmtDinheiro(r.custoUnitario)}</span>
                <span style="font-size: 0.8rem; margin-top: 0.3rem;"><strong>Rende:</strong> ${r.rende || 1} un. por fornada</span>
                <span style="font-size: 0.8rem;"><strong>Filamentos:</strong> ${fillTxt}</span>
...
function atualizarSelectProducao() {
    ...
    DB.receitas.forEach(r => {
        const opt = document.createElement('option'); opt.value = r.id; opt.textContent = `${r.nome} (Custo Unitário: ${fmtDinheiro(r.custoUnitario)})`; sel.appendChild(opt);
    });
}
...
document.getElementById('form-producao').addEventListener('submit', async (e) => {
    ...
    const qtdPerdida = parseInt(prompt(`Das ${qtdTotalProduzida} unidades, quantas deram erro/foram descartadas?`, "0")) || 0;
